...
public class FlightBookingController {
	...
	public String getFlights(@ModelAttribute("logInForm") LogInDetails logInForm, @ModelAttribute("userForm") User user, @ModelAttribute("flightSelected") FlightSelected flightSelected, @ModelAttribute ("flightSearch") FlightSearch flightSearch, Model model) {
		...
		flightSearch.setSeats(1);//min one seat
		return "flightpage";
...
//	}
	...
	@RequestMapping(value="flightWeWishToFly", method=RequestMethod.POST)
	public String flyAirline(@ModelAttribute("flightSelected") FlightSelected flightSelected, Model model, BindingResult result, HttpSession session) {
		System.out.println(flightSelected.getFlight_id());
		flightSelected.setFlightSelected(true);
		return "redirect:makePayment.html";
	}
